...
test("appendPromptUpdateEntry injects deterministic changed bullet", () => {
  const source = [
    "# Changelog",
    "",
    "## [Unreleased]",
  ...
  const updated = appendPromptUpdateEntry(source, "2026-03-12");
  assert.match(updated, /^### Changed$/m);
  assert.match(updated, /Prompt update generated by deploy script \(2026-03-12\)/);
  const rerun = appendPromptUpdateEntry(updated, "2026-03-12");
