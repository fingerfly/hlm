...
function normalizeCandidates(candidates, finalLabel) {
  ...
  for (const item of source) {
    ...
    out.push({ label, score: Number.isFinite(score) ? Math.max(0, Math.min(1, score)) : 0 });
  }
  if (finalLabel && !seen.has(finalLabel)) out.push({ label: finalLabel, score: 1 });
  out.sort((a, b) => b.score - a.score);
  return out.slice(0, 3);
}

function normalizeUncertainIndices(indices) {
  if (!Array.isArray(indices)) return { ok: false, value: [], problem: "uncertainIndices must be an array" };
  const cleaned = [...new Set(indices)];
  if (!cleaned.every((i) => Number.isInteger(i) && i >= 0 && i <= 13)) {
    return { ok: false, value: [], problem: "uncertainIndices must be integers in 0..13" };
  }
...
export function normalizeVisionResult(input = {}) {
  ...
  if (!ALLOWED_STATUS.has(status)) problems.push("status must be accepted|need_human_confirm|failed");
